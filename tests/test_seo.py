...
def test_a_player_page_names_the_shape_its_notes_are_about(standard_corpus,
                                                           profiled):
    """The notes under the profile tables say what these drafts ARE, so that
    a reader can tell the twelve-team assumption the finish figures use from
    the corpus's own shape. Half a shape is the half that misleads: "4-team"
    over a standard-scoring corpus reads as the PPR one every other page on
    this site used to be."""
    seo.clear_pages()
    body = html.unescape(_client(profiled).get("/adp/dandre-swift").text)

    assert "4-team standard" in body
    assert "twelve-team league's shape" in body
    assert "(4-team standard, 6 rounds)" in body      # the provenance line
